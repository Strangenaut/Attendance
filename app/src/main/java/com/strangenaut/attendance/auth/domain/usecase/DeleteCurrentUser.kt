...
class DeleteCurrentUser(private val repository: AuthRepository) {

    suspend operator fun invoke() {
        when (val deletionResponse = repository.revokeAccess()) {
            is Response.Success -> Unit
            is Response.Failure -> throw Exception(deletionResponse.message)
        }
    }
}